...
import yaml
import datetime
from lib.evestatic import evejumps
import time
from lib.evestatic import evestatic	
import threading	
...
print(end - start)
print len(g_orders)
with open(pkg_resources.resource_filename(__name__,'dumps/market_dump.json'), 'wb') as outfile:
	json.dump(g_orders, outfile)
start = time.time()
